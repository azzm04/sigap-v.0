...
// Dibuat DETERMINISTIK (idJaminan yang sama selalu menghasilkan token yang sama) supaya revalidatePath(`/gl/${token}`) di app/gl/[idJaminan]/actions.ts
// selalu cocok persis dengan URL yang sedang dibuka pengguna -- kalau
// token acak setiap kali di-generate, revalidatePath akan menyasar path yang salah. IV diturunkan dari HMAC(kunci, idJaminan) supaya deterministik
