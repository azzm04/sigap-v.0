...
export function FilterPeringatan({
  ...
  opsi: { loket: string[]; tahapProses: string[] };
  ukuran: number;
  basePath?: string;
}) {
  ...
  return (
    <div className="border-b border-border bg-muted/40 p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-end sm:gap-2">
        ...
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="tahap_proses">Tahap Proses</Label>
          <Select
            id="tahap_proses"
            value={nilai.tahap_proses ?? ""}
            onChange={(e) => terapkan({ tahap_proses: e.target.value || undefined })}
            placeholder="Semua"
            options={opsi.tahapProses}
